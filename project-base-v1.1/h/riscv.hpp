...
public:
    ...
    static uint64 r_cstatus();

    // write register sstatus
    static void w_cstatus(uint64 sstatus);
...
inline uint64 Riscv::r_cstatus()
{
    uint64 volatile sstatus;
    __asm__ volatile ("csrr %[CONSOLE_STATUS], sstatus" : [CONSOLE_STATUS] "=r"(sstatus));
    return sstatus;
}

inline void Riscv::w_cstatus(uint64 sstatus)
{
    __asm__ volatile ("csrw sstatus, %[CONSOLE_STATUS]" : : [CONSOLE_STATUS] "r"(sstatus));
}
